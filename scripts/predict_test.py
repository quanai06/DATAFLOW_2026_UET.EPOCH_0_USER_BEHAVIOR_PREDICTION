...
# Thay thế hàm ensemble_predict cũ bằng hàm này
def ensemble_predict_proba(models, X_seq, X_mask, device):
    for m in models:
        m.eval()
    ...
    with torch.no_grad():
        ...
        outputs_sum = None
        # Trọng số nội bộ của các fold Transformer (nếu bạn muốn giữ nguyên logic cũ)
        weights = [1, 1, 1, 1, 1, 0.5] 
        
        # 1. Cộng dồn Logits từ các model Transformer
        for m, w in zip(models, weights):
            outputs = m(X_seq, X_mask) # outputs là list 6 tensor (cho 6 attr)

            if outputs_sum is None:
                outputs_sum = [out * w for out in outputs]
            else:
                for i in range(len(outputs)):
                    outputs_sum[i] += outputs[i] * w

        # 2. Chuyển Logits -> Probabilities (Softmax)
        probs_list = []
        for out in outputs_sum:
            # Softmax theo chiều class (dim=1)
            p = torch.nn.functional.softmax(out, dim=1).cpu().numpy()
            probs_list.append(p)

    return probs_list # Trả về list chứa 6 mảng numpy

def get_combine_probs_only():
    print("⏳ [COMBINE] Loading models...")
    CFG = {
        'MAX_LEN': 37,
        'MODEL_DIR': 'models/combine', # Hoặc 'models/lstm' tuỳ bạn chọn folder nào tốt nhất
        'DATA_PATH': 'data'
    }
    
    # Load Data & Preprocess (Copy từ code cũ của bạn)
    df_test = pd.read_csv(f"{CFG['DATA_PATH']}/layer2/X_test.csv")
    X_test_seq = tc.process_sequences_val(tc.get_sequences(df_test), CFG['MAX_LEN'])
    X_test_stats = tc.get_stats(df_test)
    ...
    import glob
    model_files = sorted(glob.glob(f"{CFG['MODEL_DIR']}/*.keras")) # Hoặc .h5
    
    if not model_files:
        print("❌ Không tìm thấy model Combine nào!")
        return None

    for m_file in model_files:
        model = tf.keras.models.load_model(m_file, safe_mode=False)
        # Predict trả về list 6 mảng (nếu model multi-output)
        preds = model.predict([X_test_seq, X_test_stats_sc], verbose=0)
        all_models_preds.append(preds)
        tf.keras.backend.clear_session()

    # Trung bình cộng xác suất các model Combine
    # all_models_preds có dạng: [Model1_Output, Model2_Output, ...]
    # Model1_Output là list [Attr1_probs, Attr2_probs, ...]
    
    avg_probs_list = []
    num_attributes = 6 
    
    for i in range(num_attributes):
        # Lấy xác suất của Attr_i từ tất cả các model
        col_preds = [m[i] for m in all_models_preds]
        # Trung bình cộng
        avg_probs = np.mean(col_preds, axis=0)
...
def main_soft_voting():
    ...
    print(f"🚀 Bắt đầu Soft Voting: Transformer ({W_TRANS}) + Combine ({W_COMB})")

    # ==========================================
    # PHẦN 1: LẤY PROBS TỪ TRANSFORMER
    # ==========================================
    print("\n1️⃣  Đang chạy Transformer...")
    feature_path = "data/layer3_features/transformer"
    ...
    for fold in range(n_splits):
        ...
        model = TransformerModel(
            vocab_size=config["vocab_size"], num_classes_list=num_classes_list,
            d_model=config["d_model"], nhead=config["nhead"],
            num_layers=config["num_layers"], dim_ff=config["dim_ff"], max_len=config["max_len"]
        )
        model.load_state_dict(ckpt["model_state_dict"])
        model.to(device)
    ...
    model_full = TransformerModel(**{k: config[k] for k in ["vocab_size","d_model","nhead","num_layers","dim_ff","max_len"]}, num_classes_list=num_classes_list)
    model_full.load_state_dict(ckpt["model_state_dict"])
    model_full.to(device)
    models_trans.append(model_full)

    # Load Data Transformer
    X_seq, X_mask = load_test_data(feature_path)
    
    # Lấy Probs (Sử dụng hàm mới viết ở bước 1)
    probs_trans_list = ensemble_predict_proba(models_trans, X_seq, X_mask, device)


    # ==========================================
    # PHẦN 2: LẤY PROBS TỪ COMBINE
    # ==========================================
    print("\n2️⃣  Đang chạy Combine (LSTM)...")
    probs_comb_list = get_combine_probs_only() # Hàm mới ở bước 2


    # ==========================================
    # PHẦN 3: TỔNG HỢP VÀ GHI FILE
    # ==========================================
    print("\n3️⃣  Đang tổng hợp kết quả...")
    
    final_preds = []
    
    # Duyệt qua 6 cột (attr_1 -> attr_6)
    for i in range(6):
        p_t = probs_trans_list[i]
        p_c = probs_comb_list[i]
        
        # Công thức Soft Voting
        p_final = (p_t * W_TRANS) + (p_c * W_COMB)
        
        # Lấy class có xác suất cao nhất
        pred_idx = np.argmax(p_final, axis=1)
        
        # Decode (Chuyển số về chữ)
        # Lưu ý: encoders[i] là dict {label: index}. Cần đảo ngược.
        mapping = encoders[i]
        inv_map = {v: k for k, v in mapping.items()}
        decoded = [inv_map[idx] for idx in pred_idx]
        
        final_preds.append(decoded)

    # Transpose để đúng format (Dòng là sample, Cột là attr)
    final_preds = np.array(final_preds).T 

    # Load ID từ file test gốc
    test_df = pd.read_csv("data/layer1_raw/X_test.csv")
    ...
    print(f"✅ Xong! Đã lưu file {filename}")
